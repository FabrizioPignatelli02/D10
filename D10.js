...
const onlyInLastMillennium = function(array)
{
  for(let i=0; i<array.length; i++)
  {
    const intYears = parseInt(array[i].Year);
    if(intYears < 2000)
    {
      console.log("Es.15:", array[i].Title);
    }
  }
}
...
const sumAllTheYears = function(array)
{
  ...
  for(let i=0; i<array.length; i++)
  {
    const yearFilm = parseInt(array[i].Year);
    sumYears+=yearFilm;
  }
  console.log("Es.16: Somma degli anni:", sumYears);
}
...
const searchByTitle = function(array, titleString)
{
  for(let i=0; i<array.length; i++)
  {
    const filmTitle = array[i].Title;
    if(filmTitle === titleString)
    {
      console.log("Es.17:", filmTitle);
    }
  }
}
...
const searchAndDivide = function(array, titleFilmString)
{
const responseArray = {match: [], unmatch: []};
const matchFilm = [];
const unmatchFilm = [];
for(let i=0; i<array.length; i++)
  {
    const filmTitle = array[i].Title;
    if(filmTitle !== titleFilmString)
    {
      unmatchFilm[i]=filmTitle;
    }
    else
    {
      matchFilm[i]=filmTitle;
    }
  }
  

  responseArray.match.push(matchFilm);
  responseArray.unmatch.push(unmatchFilm);

  console.log("Es.18:", responseArray);


}

searchAndDivide(movies, "Avengers: Endgame");
...
const removeAllLiFromMyList = function()
{
  const ul = document.getElementById("myList");
  ul.innerHTML = "";
}
